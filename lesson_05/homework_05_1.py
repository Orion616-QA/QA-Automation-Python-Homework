...
filtered_cars = [
  (name, specification) for name, specification in car_data.items()
  if specification[1] >= year_min and specification[2] >= engine_min and specification[4] <= price_max
]

sorting_cars = sorted(filtered_cars, key=lambda x: x[1][4])

for car_name, (color, year, engine, car_type, price) in sorting_cars[:5]:
  print(f"{car_name}: color - {color}, year - {year}, engine - {engine}, car type - {car_type}, price - {price}")